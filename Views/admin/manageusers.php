...
<style>
	#errorcl {
		...
		color: red;
		border: 100px white;
		text-shadow: 0 0 5px #FFF, 0 0 4px #20c997;
		display: none;
	}
</style>
			...
			<div class="col-12">
				...
				<div class="col backTheam cRound p-4">
					<div class="row pb-2 pt-2">
						...
						<div class="col-md-6">
							<label for="username" class="form-label">Username<strong id="errorcl" class="errorcl">user name</strong></label>
		...
		$.ajax({
			...
			success: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					table += '<td><button class="delete-user-btn" data-eid="' + data[i].admin_id + '">Delete</button></td>';
	...
	$("#createBtn").click(function(e) {
		...
		$.ajax({
			...
			success: function(response) {
				if (response.message === "Admin and privileges added successfully") {
					Swal.fire({
						title: 'Success!',
...
<script>
	...
	function chengepw(id) {
		Swal.fire({
			...
			preConfirm: (password) => {
				if (!validatePassword(password)) {
					Swal.showValidationMessage("Password does not meet criteria");
				} else {
		...
		Swal.fire({
			...
			if (result.isConfirmed && result.value) {
				...
				if (result.value.status === 'success') {
					...
					Swal.fire({
						title: 'Error!',
						text: result.value.message || 'There was an error deleting the user',
						icon: 'error',
						confirmButtonText: 'OK'
					});
				}
...
<script>
	...
	//username validation
	$("#username").change("input", function() {
		var username = $(this).val();
		$.ajax({
			url: "../../admin/getdata/getusername.php",
			type: "POST",
			data: {
				username: username
			},
			success: function(response) {
				console.log(response); // Check the actual response
				if (response == true) {
					$("#username").css("border", "1px solid red");
					$("#createBtn").prop("disabled", true);
					$("#createBtn").css("cursor", "not-allowed");
					$("#errorcl").text("Username already exists");
					$("#errorcl").css("display", "block");
				} else {
					$("#username").css("border", "1px solid green");
					$("#createBtn").prop("disabled", false);
					$("#errorcl").css("display", "none");
					$("#createBtn").css("cursor", "default");
