...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === "search") {
    ...
    $query = $conn->prepare("SELECT e.id AS 'empid', e.first_name AS 'Name', e.NIC, e.employee_no AS 'Employee No', e.phone_no AS 'Mobile_Number', c.name_en AS 'City', d.name_en AS 'District', b.name AS 'Bank Name' FROM employee e INNER JOIN bankbranches bb ON e.BankCode = bb.branchID INNER JOIN banks b ON bb.bankID = b.id INNER JOIN cities c ON e.postcode = c.postcode INNER JOIN districts d ON c.district_id = d.id WHERE e.employee_no LIKE ? AND e.EmpStatus=1");
